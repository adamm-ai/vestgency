...
router.get('/', async (req, res) => {
  try {
    ...
    if (minPrice) where.priceNumeric = { ...where.priceNumeric, gte: BigInt(minPrice as string) };
    if (maxPrice) where.priceNumeric = { ...where.priceNumeric, lte: BigInt(maxPrice as string) };
