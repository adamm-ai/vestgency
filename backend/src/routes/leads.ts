...
const createLeadSchema = z.object({
  ...
  source: z.enum(['CHATBOT', 'WEBSITE_FORM', 'PHONE', 'EMAIL', 'WALK_IN', 'REFERRAL', 'SOCIAL_MEDIA', 'OTHER']).optional(),
...
const calculateScore = (lead: any): number => {
  ...
  if (lead.transactionType) score += 5;

  // Urgency
  if (lead.urgency === 'CRITICAL') score += 15;
  else if (lead.urgency === 'HIGH') score += 10;
  else if (lead.urgency === 'MEDIUM') score += 5;

  // Source quality
  if (lead.source === 'REFERRAL') score += 10;
  else if (lead.source === 'WEBSITE_FORM' || lead.source === 'CHATBOT') score += 5;
...
router.put('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (updates.budgetMin !== undefined) updateData.budgetMin = updates.budgetMin ? BigInt(updates.budgetMin) : null;
    if (updates.budgetMax !== undefined) updateData.budgetMax = updates.budgetMax ? BigInt(updates.budgetMax) : null;
    if (updates.notes !== undefined) updateData.notes = updates.notes;
    if (updates.chatMessages !== undefined) updateData.chatMessages = updates.chatMessages;
